...
function LeaderboardContent({ entries }: LeaderboardContentProps) {
  if (entries.length === 0) {
    return (
...
export default function LeaderboardPage() {
  ...
  const pageWrapper = (children: React.ReactNode) => (
    <div className="min-h-screen bg-brand-primary-dark flex flex-col relative overflow-hidden">
      <div className="absolute inset-0 h-full w-full bg-[linear-gradient(to_right,#002965_1px,transparent_1px),linear-gradient(to_bottom,#002965_1px,transparent_1px)] bg-[size:45px_45px]" />
      <Header />
      {children}
      <Footer />
    </div>
  ...
  // ── Desktop / Signage layout ───────────────────────────────

  if (variant === "desktop") {
    return pageWrapper(
      <main className="flex-1 flex flex-col justify-center items-center px-16 py-6 gap-8 relative z-10">
        <div className="flex items-center gap-4">
          <Link href="/" className="p-2 text-brand-primary opacity-70 hover:opacity-100 transition-opacity">
  ...
  return pageWrapper(
    <main className="flex-1 flex flex-col items-center justify-center px-6 py-6 gap-6 max-w-lg mx-auto w-full relative z-10">
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.96 }}
        className="relative flex items-center justify-center px-14 py-4 font-mono text-xl uppercase tracking-[0.2em] bg-[#0a192f] text-brand-primary border-2 border-brand-primary transition-all cursor-pointer"
      >
